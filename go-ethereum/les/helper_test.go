...
import (
	...
	"github.com/haxicode/go-ethereum/consensus/ethash"
	...
	"github.com/haxicode/go-ethereum/eth"
	"github.com/haxicode/go-ethereum/ethdb"
	"github.com/haxicode/go-ethereum/event"
	"github.com/haxicode/go-ethereum/les/flowcontrol"
	"github.com/haxicode/go-ethereum/light"
	"github.com/haxicode/go-ethereum/p2p"
	"github.com/haxicode/go-ethereum/p2p/discover"
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		tx, _ := types.SignTx(types.NewTransaction(types.Binary, block.TxNonce(testBankAddress), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		tx1, _ := types.SignTx(types.NewTransaction(types.Binary, block.TxNonce(testBankAddress), acc1Addr, big.NewInt(1000), params.TxGas, nil, nil), signer, testBankKey)
		tx2, _ := types.SignTx(types.NewTransaction(types.Binary, nonce, acc2Addr, big.NewInt(1000), params.TxGas, nil, nil), signer, acc1Key)
	//	tx3, _ := types.SignTx(types.NewContractCreation(0, big.NewInt(int64(nonce+1)), 0, big.NewInt(200000), testContractCode), signer, acc1Key)
		testContractAddr = crypto.CreateAddress(acc1Addr, nonce+1)
	//	tx4, _ := types.SignTx(types.NewContractCreation(0, big.NewInt(int64(nonce+2)), 0, big.NewInt(200000), testEventEmitterCode), signer, acc1Key)
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(types.Binary, block.TxNonce(testBankAddress), testContractAddr, big.NewInt(0), 100000, nil, data), signer, testBankKey)
		block.AddTx(tx)
	case 3:
